    ...
    private synchronized void updateResult(final Result result) {
        ...
        switch (lowerCaseName = result.getStatus().lowerCaseName()) {
            case "failed": {
                ...
                if(typeofError.equalsIgnoreCase(""))
                {
                	//Base.suiteXLS.setCellData("TestSuite", "Investigations", Base.rowcount, errorMessage );
                }else
                {
                	//Base.suiteXLS.setCellData("TestSuite", "Investigations", Base.rowcount, result.getError().toString() );
                }
